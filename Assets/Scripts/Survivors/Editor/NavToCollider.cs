...
namespace Survivors.Editor
{
    ...
    public class NavToCollider : UnityEditor.Editor
    {
        ...
        void OnSceneGUI()
        {
            if (Event.current.type == EventType.Repaint) OnShowVerts();
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            // EditorGUILayout.Space();
            // if (GUILayout.Button("Create Box from Vertices")) GenBoxCollider();

            // EditorGUILayout.Space();
            //
            // if (GUILayout.Button("Clear Children"))
            //     for (var i = 0; i < m_NavToCollider.transform.childCount; i++)
        ...
        void GenBoxCollider(bool xAxis = false)
        {
            for (var i = 0; i < m_NavToCollider.transform.childCount; i++)
                DestroyImmediate(m_NavToCollider.transform.GetChild(i).gameObject);
            ...
            var gridWidth = Mathf.CeilToInt((bounds.max.x - bounds.min.x) / voxelSize);
            var gridHeight = Mathf.CeilToInt((bounds.max.z - bounds.min.z) / voxelSize);

            var occupiedGrid = new bool[gridWidth, gridHeight];



            for (var y = bounds.min.z; y < bounds.max.z; y += voxelSize)
            for (var x = bounds.min.x; x < bounds.max.x; x += voxelSize)
            {
                ...
                if (NavMesh.SamplePosition(samplePos, out var hit, voxelSize, NavMesh.AllAreas))
                {
            ...
            var processedGrid = new bool[gridWidth, gridHeight];
            var foundRectangles = new List<RectInt>();
            ...
            for (var x = 0; x < gridWidth; x++)
                ...
                if (occupiedGrid[x, y] && !processedGrid[x, y])
                {
                    ...
                    // --- Expand Down ---
                    var currentHeight = 0;
                    for (var cy = y; cy < gridHeight; cy++)
                    {
                        var rowIsValid = true;
                        // Check all cells in this row across the determined width
                        for (var cx = x; cx < x + currentWidth; cx++)
                    ...
                    var newRect = new RectInt(x, y, currentWidth, currentHeight);
                    foundRectangles.Add(newRect);

                    for (var py = y; py < y + currentHeight; py++)
                    for (var px = x; px < x + currentWidth; px++)
        ...
        public static void SimplifyMeshTopology(List<Vector3> vertices, List<int> indices, float weldThreshold)
        {
            ...
            for (var i = 0; i < vertices.Count; ++i)
            {
                ...
                if (vertexBuckets.TryGetValue(currentVertex, out var indexRefs))
                {
                    ...
                    if (minRepeatedIndex == 0) minRepeatedIndex = i;
                }
                else
                {
            ...
            Debug.Log(
                $"Finished simplifying mesh topology. Time: {Time.realtimeSinceStartup - startTime}. initVerts: {startingVerts}, endVerts: {vertices.Count}");
