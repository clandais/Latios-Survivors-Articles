...
using Survivors.Play.Authoring.Debug;
using Unity.Burst;
...
namespace Survivors.Play.Systems.Debug
{
    ...
    public partial struct CaptureDisplaySystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            foreach ((var pair, Entity entity) in Query<RefRO<CapturePairTarget>>().WithEntityAccess())
            {
                if (!Exists(pair.ValueRO.colliderA) || !Exists(pair.ValueRO.colliderB))
                    continue;
                if (!HasComponent<Collider>(pair.ValueRO.colliderA) || !HasComponent<Collider>(pair.ValueRO.colliderB))
                    continue;
                if (!HasComponent<WorldTransform>(pair.ValueRO.colliderA) || !HasComponent<WorldTransform>(pair.ValueRO.colliderB))
                    continue;

                // var go = debug.GetAuthoringObjectForEntity(entity) as UnityEngine.GameObject;
                // var playbacker = go?.GetComponent<Playbacker>();
                if (pair.ValueRO.drawOperation == DrawOperation.None) // && playbacker == null)
                    continue;

                var colliderA = GetComponent<Collider>(pair.ValueRO.colliderA);
                var transformA = GetComponent<WorldTransform>(pair.ValueRO.colliderA).worldTransform;
                var colliderB = GetComponent<Collider>(pair.ValueRO.colliderB);
                var transformB = GetComponent<WorldTransform>(pair.ValueRO.colliderB).worldTransform;

                var aabbA = Latios.Psyshock.Physics.AabbFrom(colliderA, transformA);
                ...
                // var hit = Latios.Psyshock.Physics.DistanceBetween(colliderA,
                //     transformA,
                //     colliderB,
                //     transformB,
                //     pair.maxDistance,
                ...
                //     Latios.Psyshock.Physics.DistanceBetweenAll(colliderA, transformA, colliderB, transformB, pair.maxDistance, ref allCollector);
                //
                ...
                //         {
                //             playbacker.closestContacts.contactPointPairs.Add(new Float3Pair
                //                 { a = closestContactsResult[i].location, b = closestFlipped[i].location });
                //         }
                //
                //
                ...
                //             playbacker.allContacts.Add(
                //                 new SerializedUnityContactsResult { contactPointPairs = new System.Collections.Generic.List<Float3Pair>() });
                //
